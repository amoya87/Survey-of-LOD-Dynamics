...
\runningtitle{Dynamics on Linked Data: A Survey}
%\subtitle{}
...
%\textbf{Link Data Growth:}

Every day more players participate in the ecosystem of linked data and the community around Linked Data continues to develop tools for data editors to share their data as Linked Data. The result of these efforts is several software libraries that convert information from arbitrary formats to RDF, for example: D2R Server3, XLWrap4, Any235, etc.
In addition, more and more Web 2.0 portals expose their data as linked data, for example, the most used content management systems such as Drupal 7 and knowledge management systems such as SemWiki, or news portals such as the New York Times or the BBC, publish their articles following the principles of Linked Data. As a result, the resources are continuously created, moved and deleted, linked and unlinked.

From the time that the LD principles have been created until now, Linked Data has grown significantly in the number of RDF datasets and the size of those datasets~\cite{SchmachtenbergBP14}. Additionally, a wide variety of applications use information sourced from the Web to offer services.

%\textbf{Dynamic Challenges:}

Web of Data continues to expand in size, scope and diversity, data editors continually try to improve the quality of their data by changing vocabularies, adding new information and relationships, or eliminating obsolete ones.

Recent investigations~\cite{UmbrichHHPD10, DividinoSGG13, AuerDML12, SchmachtenbergBP14} have shown that the data published and interlinked on the Web is subject to frequent changes. The volume and velocity of changes constitute important challenges that applications must handle to keep results up-to-date.
...
\begin{figure*}[h]
	\centering
	\includegraphics[scale=0.8]{methodology.pdf}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcc}
		...
		Talis Changesets \cite{Tunnicliffe05} & \cmark   & \\		
